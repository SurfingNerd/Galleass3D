...
contract LandLib is Galleasset, DataParser {
  ...
  function generateLand() onlyOwner isBuilding public returns (bool) {
    ...
    bytes32 id = keccak256(abi.encodePacked(nonce++,blockhash(block.number-1)));
    uint16 x = getUint16(id[0], id[1]);
    uint16 y = getUint16(id[2], id[3]);
    bytes32 landParts1 = keccak256(abi.encodePacked(id));
    bytes32 landParts2 = keccak256(abi.encodePacked(landParts1));
  ...
  public
  {
    ...
    for(uint8 index = 0; index < 18; index++){
      uint16 thisUint16 = getUint16(landParts1[index], landParts2[index]);
  ...
  function _buyTile(address _sender, uint _amount, bytes memory _data) internal returns (bool) {
    ...
    address landContractAddress = getContract("Land");
    Land landContract = Land(landContractAddress);
    address copperContractAddress = getContract("Copper");
    require(msg.sender == copperContractAddress, 'sender must be coppercontract');
    ...
    if(landContract.ownerAt(_x,_y,_tile)==address(0)){
      ...
      require(_amount>=OPENLANDTILECOST, 'must send at least BLANK LAND TILE COST');
      //transfer the copper to the Land Contract
      require(copperContract.transfer(landContractAddress ,OPENLANDTILECOST), 'unable to transfer copper');
    }else{
      //must be for sale
      require(landContract.priceAt(_x,_y,_tile)>0, 'must be for sale');
      //must send at least enough as the price
      require(_amount>=landContract.priceAt(_x,_y,_tile),'must send at least enough as the price');
      //transfer the copper to the Land Owner
      require(copperContract.transfer(landContract.ownerAt(_x,_y,_tile),_amount), 'failed: transfer the copper to the Land Owner');
    }
  ...
  function buildTile(uint16 _x, uint16 _y,uint8 _tile,uint16 _newTileType) public isGalleasset("LandLib") returns (bool) {
    ...
    require(msg.sender==landContract.ownerAt(_x,_y,_tile), 'must be the owner of the tile to build here');
    ...
    if(tileType==tileTypes["MainHills"]||tileType==tileTypes["MainGrass"]){
      ...
      if(_newTileType==tileTypes["Castle"]){
        ...
        require(stoneContract.transferFrom(msg.sender,getContract("Land"),20), 'failed to transfer Stones'); //use 20 stone (moved to Land contract)
        //set tile type on the Land Contract
        landContract.setTileTypeAt(_x,_y,_tile,_newTileType);
        //set contract at tile to the Village
  ...
  function _buildTimberCamp(address _sender, uint _amount, bytes memory _data) internal returns (bool) {
    ...
    require(landContract.ownerAt(_x,_y,_tile)==_sender, 'sender must own the tile');
    //they must send in 6 copper
    require(_amount>=6, 'they must send in 6 copper');
    //move that copper to the Land contract
    StandardTokenInterface copperContract = StandardTokenInterface(copperContractAddress);
    require(copperContract.transfer(getContract("Land"),_amount), 'failed: move that copper to the Land contract');
    //must be built on a forest tile
    require(landContract.tileTypeAt(_x,_y,_tile)==tileTypes["Forest"], 'must be built on a forest tile');
    ...
    require(_useCitizenAsLumberjack(_sender,_x,_y,_tile,citizenId), 'failed: _useCitizenAsLumberjack');
    //set new tile type
  ...
  function _useCitizenAsLumberjack(address _sender, uint16 _x, uint16 _y, uint8 _tile,uint _citizen) internal returns (bool){
    ...
    require(owner==_sender, 'sender must own citizen');
    //citizen must be idle
    require(status==1, 'citizen must be idle');
    //citizen must be at location
    require(_x==x, '_x==x');
    require(_y==y, '_y==y');
    require(_tile==tile, '_tile==tile');
    ...
    require(strength>1, 'requires strength > 1');
    require(stamina>1, 'requires stamina > 1');
    //set citizen status to TimberCamp tile type (lumberjack!)
    require(citizensLibContract.setStatus(_citizen,uint8(tileTypes["TimberCamp"])), 'unable to set status to TimberCamp');
    return true;
  }
  ...
  function _extractRawResource(address _sender, uint _amount, bytes memory _data) internal returns (bool) {
    ...
    //they must own the tile
    require(landContract.ownerAt(_x,_y,_tile) == _sender, 'they must own the tile');
    //move that copper to the Land contract
    StandardTokenInterface copperContract = StandardTokenInterface(copperContractAddress);
    require(copperContract.transfer(getContract("Land"),_amount), 'move that copper to the Land contract');
    ...
    if(landContract.tileTypeAt(_x,_y,_tile)==tileTypes["Forest"] || landContract.tileTypeAt(_x,_y,_tile)==tileTypes["TimberCamp"]){
      ...
      require(_amount>=3, 'they must send in 3 copper to extract 1 Timber');
      resourceContract = StandardTokenInterface(getContract("Timber"));
      require(resourceContract.mint(_sender,1), 'failed: minting timber.');
      return true;
      ...
      require(_amount>=2, 'must send in 2 copper to extract 1 Greens');
      resourceContract = StandardTokenInterface(getContract("Greens"));
      require(resourceContract.mint(_sender,1), 'minting greens failed');
      return true;
    } else if(landContract.tileTypeAt(_x,_y,_tile)==tileTypes["Mountain"]) {
      //they must send in 4 copper to extract 1 Stone
      require(_amount>=4,'must send in 4 copper to extract 1 Stone');
      resourceContract = StandardTokenInterface(getContract("Stone"));
      require(resourceContract.mint(_sender,1), 'minting stone failed.');
      return true;
    }else{
      return false;
    }
