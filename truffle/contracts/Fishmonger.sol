...
contract Fishmonger is StandardTile {
  ...
  function _restockBay(StandardTokenInterface fishContract,uint16 _x,uint16 _y,address _species,uint256 _amount) internal {
  ...
  function _buyFillet(address _sender, uint _amount, bytes _data) internal returns (bool) {
    ...
    //transfer fillets
    StandardTokenInterface filletContract = StandardTokenInterface(filletAddress);
