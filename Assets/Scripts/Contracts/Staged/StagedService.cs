...
using Galleass3D.Contracts.Staged.ContractDefinition;

namespace Galleass3D.Contracts.Staged
{
    public partial class StagedService
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.Web3 web3, StagedDeployment stagedDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<StagedDeployment>().SendRequestAndWaitForReceiptAsync(stagedDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.Web3 web3, StagedDeployment stagedDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<StagedDeployment>().SendRequestAsync(stagedDeployment);
        }

        public static async Task<StagedService> DeployContractAndGetServiceAsync(Nethereum.Web3.Web3 web3, StagedDeployment stagedDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, stagedDeployment, cancellationTokenSource);
            return new StagedService(web3, receipt.ContractAddress);
        }
        ...
        public StagedService(Nethereum.Web3.Web3 web3, string contractAddress)
        {
            Web3 = web3;
