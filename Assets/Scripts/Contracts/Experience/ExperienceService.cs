...
using Galleass3D.Contracts.Experience.ContractDefinition;

namespace Galleass3D.Contracts.Experience
{
    public partial class ExperienceService
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.Web3 web3, ExperienceDeployment experienceDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<ExperienceDeployment>().SendRequestAndWaitForReceiptAsync(experienceDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.Web3 web3, ExperienceDeployment experienceDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<ExperienceDeployment>().SendRequestAsync(experienceDeployment);
        }

        public static async Task<ExperienceService> DeployContractAndGetServiceAsync(Nethereum.Web3.Web3 web3, ExperienceDeployment experienceDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, experienceDeployment, cancellationTokenSource);
            return new ExperienceService(web3, receipt.ContractAddress);
        }
        ...
        public ExperienceService(Nethereum.Web3.Web3 web3, string contractAddress)
        {
            Web3 = web3;
            ContractHandler = web3.Eth.GetContractHandler(contractAddress);
        }

        public Task<bool> ExperienceQueryAsync(ExperienceFunction experienceFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<ExperienceFunction, bool>(experienceFunction, blockParameter);
        }

        
        public Task<bool> ExperienceQueryAsync(string returnValue1, ushort returnValue2, BlockParameter blockParameter = null)
        {
            var experienceFunction = new ExperienceFunction();
                experienceFunction.ReturnValue1 = returnValue1;
                experienceFunction.ReturnValue2 = returnValue2;
