...
using Galleass3D.Contracts.Galleasset.ContractDefinition;

namespace Galleass3D.Contracts.Galleasset
{
    public partial class GalleassetService
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.Web3 web3, GalleassetDeployment galleassetDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<GalleassetDeployment>().SendRequestAndWaitForReceiptAsync(galleassetDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.Web3 web3, GalleassetDeployment galleassetDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<GalleassetDeployment>().SendRequestAsync(galleassetDeployment);
        }

        public static async Task<GalleassetService> DeployContractAndGetServiceAsync(Nethereum.Web3.Web3 web3, GalleassetDeployment galleassetDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, galleassetDeployment, cancellationTokenSource);
            return new GalleassetService(web3, receipt.ContractAddress);
        }
        ...
        public GalleassetService(Nethereum.Web3.Web3 web3, string contractAddress)
        {
            Web3 = web3;
